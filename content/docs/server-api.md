...
| `Open77.players.identifiers(playerId)` | None, plus `players.identity.sensitive` for `endpoint` | `-> table \| nil, reason` | `open77` and `userId` (the same installation identity GUID), `name`, `fingerprint`, `joinedAt`, plus verified `license`, `steam` and `gog` when available since `2.31.13+op77.101`. `license` is the permanent Open77 account GUID without dashes; `steam` is lowercase hexadecimal and `gog` decimal. All are strings without type prefixes. `endpoint` requires the capability. See [account and store identifiers](connection-control.md#steam-gog-and-permanent-account-identifiers). |
| `GetPlayerIdentifierByType(playerId, type)` | None | `-> string \| nil, reason` | Bare identifier, with case-insensitive type: `open77`, `userId`, `name`, `fingerprint`, `license`, `steam`, `gog`. Missing ownership fields return `identifier_not_linked`; unsupported types return `unknown_identifier_type`; non-string types return `invalid_identifier_type`. |
| `GetPlayerIdentifiers(playerId)` | None | `-> array \| nil, reason` | Available identifiers as `type:value` strings, including `license:`, `steam:` and `gog:` since `2.31.13+op77.101`. Missing entries are omitted. Unknown session: `player_not_found`. |
...
| `Open77.players.identity` / `GetPlayerIdentity` | None | `(playerId)` | `{ userId, name, publicKey, fingerprint, joinedAt, license?, steam?, gog? }` for an admitted player, or `nil`. Ownership fields come from the verified Master ticket since `2.31.13+op77.101`. |
...
| `Open77.players.identifier` | None | `(playerId)` | Persistent installation identity GUID (`userId`) or `nil`; use `license` from `identifiers` for account-level persistence. |
| `Open77.players.identity` / `GetPlayerIdentity` | None | `(playerId)` | `{ userId, name, publicKey, fingerprint, joinedAt, license?, steam?, gog? }` or `nil`; the fingerprint matches the client's `identity.dump`. Ownership fields require `2.31.13+op77.101` or later. |
