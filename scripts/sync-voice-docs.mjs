/** Targeted wiki sync: preserve newer unrelated site guides/API cards. */
...
if (check) {
  ...
  assert.deepEqual(sorted(current), sorted(incoming), "Lipsync API drift");
  ...
  console.log("Voice sync verified: two guides and four API cards.");
} else {
  ...
  manifest.partialSync = { tool: "scripts/sync-voice-docs.mjs", syncedAt: manifest.syncedAt,
  ...
  console.log("Synced two voice guides and four lipsync API cards; unrelated content preserved.");
}
