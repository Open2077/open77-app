...
(async () => {
  ...
  try {
    ...
    assert.deepEqual(calls[0],{method:'PUT',body:{official:true,featured:true,featuredOrder:3,revision:0},authorization:'Bearer synthetic-directory-test'});
    reply('ok'); await page.getByRole('button',{name:'Saved',exact:true}).waitFor();
    assert.match(await page.locator('[role="status"]').allTextContents().then(x=>x.join(' ')),/Directory labels saved/);
    ...
    assert.equal(calls.length,2);assert.deepEqual(errors,[]);console.log('PASS: admin directory labels, busy feedback, exact payload, revision conflict, search, and role gate.');
