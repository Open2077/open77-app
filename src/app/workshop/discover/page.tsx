...
export const metadata = pageMetadata({ title: "Workshop", description: "Discover and share resources, gamemodes, maps and interfaces built for OPEN//77 servers.", path: "/workshop/discover" });
...
export default async function CommunityPage() {
  ...
  const sections = [
    { title: "Featured", href: "/workshop?sort=featured", featured: true },
    { title: "Trending this week", href: "/workshop?sort=trending" },
    { title: "New in the library", href: "/workshop?sort=new" },
    { title: "Recently updated", href: "/workshop?sort=updated" },
    { title: "Showcases", href: "/workshop?kind=showcase" },
  ].map((section, index) => {
  ...
  return <HubShell>
    <header className="hub-page-head hub-page-head-entry"><div><p className="hub-kicker">RESOURCES FOR SERVER BUILDERS</p>
      ...
      <div className="hub-page-head-actions"><Link className="btn btn-primary" href="/workshop">Browse the Workshop</Link><Link className="btn btn-ghost" href="/docs/server-resources">How resources work</Link></div></header>
    <form className="hub-toolbar" action="/workshop" role="search" aria-label="Search the community library">
    ...
    <nav className="hub-modes" aria-label="Categories">{categories.map(category =>
      <Link key={category.id} className="hub-chip" href={`/workshop?category=${category.id}`}>{category.label}</Link>)}
      <Link className="hub-chip" href="/workshop?kind=showcase">Showcases</Link></nav>
    ...
    <section className="hub-ctas">
      ...
      <div className="hub-cta"><p className="hub-kicker">FOR SERVER OWNERS</p><h2>Install with Warden</h2><p>Browse the Workshop from your server console, review the plan, then install, update or roll back with one confirmation.</p><Link className="btn btn-ghost btn-small" href="/docs/community-hub-warden">Read the Warden guide</Link></div>
