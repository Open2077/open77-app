...
export default async function CommunityPage() {
  ...
  return <HubShell>
    <header className="hub-page-head hub-page-head-entry"><div><p className="hub-kicker">RESOURCES FOR SERVER BUILDERS</p>
      ...
      <div className="hub-page-head-actions"><Link className="btn btn-primary" href="/workshop/browse">Browse the Workshop</Link><Link className="btn btn-ghost" href="/docs/server-resources">How resources work</Link></div></header>
    <form className="hub-toolbar" action="/workshop/browse" role="search" aria-label="Search the community library">
    ...
    <nav className="hub-modes" aria-label="Categories">{categories.map(category =>
      <Link key={category.id} className="hub-chip" href={`/workshop/browse?category=${category.id}`}>{category.label}</Link>)}
      <Link className="hub-chip" href="/workshop/browse?kind=showcase">Showcases</Link></nav>
