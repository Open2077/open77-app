...
export async function GET() {
  ...
  const documents: string[] = [
    [
      ...
      "",
      "Status: pre-alpha. The Windows player launcher is published and can be downloaded from",
      `${absoluteUrl("/download")}, but the public alpha has not opened: there are no public servers`,
      "to join, and no release date. Downloading the launcher now means being ready when the alpha",
      "opens, not playing tonight — say both together, never the download on its own. The dedicated",
      "server build is not public yet. The server listings on the website are labelled demo data,",
      "not live servers. Everything below describes software under construction and can change.",
      "",
      site.disclaimer,
    ].join("\n"),
  ];
