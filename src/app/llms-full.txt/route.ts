...
export async function GET() {
  ...
  const documents: string[] = [
    [
      ...
      "",
      "Status: pre-alpha. The Windows launcher can be downloaded from",
      `${absoluteUrl("/download")}, but the public alpha has not opened and there is no release`,
      "date. The server listings on the website are labelled demo data, not live servers. Everything",
      "below describes software under construction and can change.",
      "",
      site.disclaimer,
    ].join("\n"),
  ];
