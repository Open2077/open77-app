...
function Entries({ token, unread }: { token: string; unread: boolean }) {
  ...
  return <section className="hub-inbox" aria-label="Your notifications">
    ...
    {page?.items.map(item => <article className="hub-activity-entry" key={item.notificationId}>
      ...
      <div className="hub-actions">{item.content.path && /^(?:\/account\/creations\/[0-9a-f-]{36}\/edit|\/resources\/[a-z0-9]+(?:-[a-z0-9]+)*\/(?:versions|discussion\?thread=[0-9a-f-]{36}))$/i.test(item.content.path) && <Link href={item.content.path}>Open creation →</Link>}
