      ...
      <div className="api-panes">
        <section className="api-detail" ref={detail} aria-label="Selected function">
          {selected ? <div key={entryKey(selected)}>
            ...
            <div className="api-signature-block"><div><span>Lua signature</span><CopyButton value={selected.signature} label="Copy" /></div><pre><code>{selected.documentedSignature || selected.callSignature ? selected.signature : <><span className="api-code-comment">-- {selected.runtime} · {selected.namespace}</span>{"\n"}{selected.qualified}({selected.params.length ? "\n" : ""}{selected.params.map((param, index) => <span key={param.name}>{"    "}<span className="api-code-param">{param.name}</span>{index < selected.params.length - 1 ? "," : ""}<span className="api-code-comment">{` -- ${param.type}${param.optional ? " (optional)" : ""}`}</span>{"\n"}</span>)})</>}</code></pre></div>
            <div className="api-detail-body">
              ...
              <div className="api-detail-links"><CopyButton value={typeof window === "undefined" ? apiExplorerHref(selected) : window.location.origin + apiExplorerHref(selected)} label="Copy link" /><Link href={selected.href}>Full namespace reference ↗</Link>{selected.usageGuideHref && <Link href={selected.usageGuideHref}>{selected.usageGuideLabel ?? "Usage guide"} ↗</Link>}{selected.guideHref && <Link href={selected.guideHref}>Read the source guide ↗</Link>}</div>
