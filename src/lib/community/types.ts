...
export type CommunityDirectoryPage = CommunityPage<CommunityProject> & { rankingAsOfUtc: string | null };
export type CommunityDirectoryQuery = { query?: string; category?: string; tags?: string; kind?: string; build?: string; hasSource?: string; sort?: string; cursor?: string; limit?: number };
export type CommunitySitemapShard = { kind: "projects" | "creators"; bucket: string };
export type CommunitySitemapEntry = { path: string; lastModifiedUtc: string };
...
export type CommunityEditorialState = { projectId: string; state: string; moderationRevision: number; featured: boolean };
export type CommunityRankingExclusion = { accountId: string; excluded: boolean; revision: number; reason: string };
...
export type CommunityReleaseFiles = { releaseId: string; totalFiles: number; items: { path: string; sizeBytes: number; sha256: string }[]; nextCursor: string | null };
